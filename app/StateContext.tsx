...
export type LocationType = { lat: number, lon: number, name: string, display_name: string, addresstype: string }
...
const StateContext = createContext<{ state: State; dispatch: React.Dispatch<Action> } | undefined>(
  undefined
);
...
export const StateProvider: React.FC<StateProviderProps> = ({ children }) => {
  ...
  const initialSearchValue = searchParams.get('q') || searchParams.get('query') || '';
  const initialLocation = searchParams.get('location') || searchParams.get('loc') || null;
  const { fetchCoordinates } = useAddressToCoordinates();

  const initialState: State = { searchValue: initialSearchValue, location: null };
