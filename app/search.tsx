...
export async function usgsWaterSearch(
	...
	rows.forEach((row: { [x: string]: any }, i: number) => {
		...
		result['summary'] = `This is a ${row["datatype"]} Station, it's located at ${row["locationname"]} (${round(row['lat'], 4)}, ${round(row['long'], 4)}).`
		try {
			const tuples = row['paramcodes'].match(/\('(.*?)', '.*?'\)/g)?.map((tupleStr: string) => {
