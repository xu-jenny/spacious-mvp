...
const PDFPanelViewer: React.FC<PdfViewerProps> = ({ fileUrl, pagesToJump }) => {
  ...
  const processAllPagesForOCR = useCallback(
  ...
  return (
    <div style={{ height: "100vh", width: "75vw", position: "relative" }}>
      <div>
        {highlights.map((h) => (
          <span key={h.id}>
            [({h.position.boundingRect.x1},{h.position.boundingRect.y1}), (
            {h.position.boundingRect.x2}, {h.position.boundingRect.y2})]
          </span>
        ))}
      </div>
